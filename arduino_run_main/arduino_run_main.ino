...
void adjustLeftWall(){
  ...
  if (distLeftCM<1000 && distRightCM<1000){
    ...
    //while there is an angle difference
    while (readingDiff > 10){
      //if either left or right does not have block, break, not to adjust angle
      distRightBlk=readRightIR()/10;
      distLeftBlk=readLeftIR()/10;
      if (distRightBlk>=2 || distLeftBlk>=2){
        break;
      }
      distLeftCM=readSensor(2);
      distRightCM=readSensor(0);
      ...
      if (distLeftCM > distRightCM){        
        motors.rotateAdjust(1, true);  //rotate left
      }
      else {
        motors.rotateAdjust(1, false);  //rotate right
      }
      //delay(50);
    }

    //while there is  a displacement
    distMidCM= readSensor(MID_SENSOR);//use mid sensor to adjust distance
    while(distMidCM>=605 ||distMidCM<=595){
      distMidCM=readSensor(MID_SENSOR);
      int movement= abs(distMidCM-600);
      if(distMidCM > 600){//backwards
        motors.moveAdjust(movement, true, true, 80.0, 80.0);
      }
      else{
        motors.moveAdjust(movement, false, false, 80.0, 80.0);
